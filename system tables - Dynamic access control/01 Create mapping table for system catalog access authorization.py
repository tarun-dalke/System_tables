...
dbutils.widgets.text("mapping_table_catalog_owner", "user")
...
spark.sql(f""" GRANT ALL PRIVILEGES ON catalog {mapping_table_catalog} TO `user` """)
...
spark.sql(f"""
            ...
            (
              SELECT 
                workspace_id,
                COLLECT_SET(bu_name) AS bu_name, 
...
spark.sql(f"""
          ...
          (workspace_id, bu_name, identity_name) VALUES
              ('4488299099255398', 'bu1', 'humans'),
              ('3858701514391246', 'bu2', 'humans');
          """)

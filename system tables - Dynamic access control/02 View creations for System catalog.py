...
dbutils.widgets.text("mapping_table_name", "workspace_to_identity_access_mapping")
dbutils.widgets.text("bu_system_catalog_owner", "user")
...
spark.sql(f""" GRANT ALL PRIVILEGES ON catalog {bu_system_catalog} TO `user` """)
...
def create_restricted_view(source_catalog_name: str, destination_catalog: str, schema_name: str, 
    ...
    spark.sql(f"""
              CREATE OR REPLACE VIEW {destination_catalog}.{schema_name}.v_{table_name} AS
                  SELECT ws_name.workspace_name, v_map.bu_name, sys.*
                  FROM {source_catalog_name}.{schema_name}.{table_name} sys
                  LEFT OUTER JOIN {mapping_table_catalog}.{mapping_table_schema}.v_{mapping_table_name} v_map
                  ON v_map.workspace_id = sys.workspace_id
                  LEFT OUTER JOIN 
                  ...
                  WHERE EXISTS (
                                SELECT 1 
                                FROM {mapping_table_catalog}.{mapping_table_schema}.{mapping_table_name} AS map 
